...
El código que escribe la IA **no se valida leyéndolo entero**: se rodea de **puertas mecánicas** que fallan solas. La regla de oro: *"las pruebas pasan"* deja de ser una afirmación **asumida** en un reporte y pasa a ser **una puerta ejecutada** que **impide cerrar la tarea** si está en rojo.

**Por qué es obligatorio (evidencia, no opinión):** el código generado por LLM introduce una vulnerabilidad OWASP Top 10 detectable en ~**45%** de los casos (Veracode 2025, >100 modelos), sin mejora en 2026; DORA 2024 mide que **+25% de adopción de IA ≈ −7,2% de estabilidad de entrega**; y los agentes hacen **reward hacking** medible: con un test en conflicto con la especificación, los modelos punteros **modifican o hardcodean el test** en tasas del ~39-76%. Los tests que escribe la propia IA **no capturan** esas clases de defecto.

## Los tres tiempos de la puerta (y qué va en cada uno)
...
## Orden dentro de la puerta de commit: que falle barato primero

Dentro del tiempo "por commit", el orden importa y no es arbitrario: **de más rápido y barato a más caro**, porque lo que descarta el cambio en milisegundos debe ir antes de gastar tiempo en lo que tarda más. **Formateo → detección de secretos → comprobación estática → tipos.** Y las **pruebas no van aquí**: van por tanda, antes de publicar — meterlas en cada commit es la puerta con peor relación entre coste y fricción, porque penaliza todos los commits para atrapar lo que solo hace falta atrapar una vez, antes del `push`.

## El `Stop` no EJECUTA las puertas: comprueba que se ejecutaron (patrón sello)

**El hook `Stop` corre al final de CADA TURNO**, no una vez por tanda. Si le cuelgas la suite o las puertas caras, cada respuesta del agente paga ese coste — y la puerta **acaba desactivada por insoportable**. *(Medido en una implantación real: suite **284 s**, puertas "rápidas" **35 s** sumados.)*
...
**Coste de cuota:** los hooks **`command` (shell) no consumen cuota de modelo**; los de tipo **`prompt`/`agent` sí**. → **Puertas deterministas primero y siempre; puertas con modelo, solo por excepción.** El coste real aparece cuando la puerta falla y el agente **itera** para arreglarlo: pon un **escape a humano tras N fallos** (evidencia: ofrecer una salida "abort/flag for human" bajó el *cheating* de un modelo puntero del 54% al 9%).

## Umbrales: defendibles vs gameables

- **Cobertura de líneas → INFORMATIVA, nunca puerta absoluta.** El 100% sin aserciones útiles es el ejemplo canónico de métrica falseable (induce tests-basura).
- **Cobertura del DIFF → sí es puerta razonable** (exigir cobertura solo en el código nuevo/tocado; **80%** como umbral de arranque). **Branch coverage** ≥70-80% en módulos nuevos como puerta suave.
- **Mutation score → la puerta de calidad-de-los-tests, y la pieza clave del modelo.** No se puede falsear ejecutando código: exige aserciones que **restrinjan** el comportamiento. Es lo único que garantiza que valen algo unos unit tests **que nadie lee**. Arranque: `break` (bloqueante) **60% por módulo crítico**, con **ratchet** (solo puede subir); global informativo; ejecución **incremental** (no full en cada edición).
- **Complejidad y tamaño → puertas duras** (p. ej. complejidad ciclomática ≤10-15 por función; función ≤40-60 líneas). Son *constraints ex ante*, no limpieza *ex post*.
...
Una puerta **mide** lo que ya se escribió y corre **después** de escribirlo. Eso da consistencia y no cuesta **un gramo de libertad**. **Nada en esta doctrina autoriza impedir escribir:** la IA es **un compañero de desarrollo más** y escribe código, **tests**, migraciones y configuración con la misma libertad que una persona del equipo. Un mecanismo que impide escribir no mide nada — solo bloquea, y bloquear no aporta consistencia: **aporta parálisis**. Si una puerta no puede expresarse como *"comprueba X sobre lo ya escrito"*, **no se instala**.
...
3. **Un commit que toca tests + implementación a la vez es una SEÑAL INFORMATIVA, no un rechazo:** es el caso normal de cualquier desarrollo (escribes el test y el código que lo satisface), y bloquearlo genera falsos positivos garantizados — *una puerta con falsos positivos acaba desactivada, que es la única forma real de perderla*.
4. **Mutation score como antifraude estructural:** aunque el agente escriba los tests, si no matan mutantes, la puerta lo detecta.
...
6. **Ratchet en baselines** (PHPStan, cobertura): la baseline **solo puede encoger**; **nunca** se auto-actualiza en la puerta (requiere commit humano explícito).
...
- **Puertas lentas fuera del camino de edición** (solo en `Stop`/DoD, e incrementales).
- **Falsos positivos:** el análisis estático genera ruido → **baseline + allowlists comentadas** para evitar la fatiga que lleva a ignorar la puerta.
...
## Lo que ninguna puerta cubre (frontera explícita)

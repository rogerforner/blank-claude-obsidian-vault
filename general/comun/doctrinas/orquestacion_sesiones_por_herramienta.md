...
> **Y una cuarta capa que muchos dan por hecha y puede no existir: LA PREGUNTA.** Si quien opera arranca las sesiones en **permisos amplios** —cosa razonable y frecuente, para no ir aprobando cada edición—, **el sistema no pregunta nunca**. Entonces todo lo que estaba escrito como *"prepara, deja listo y para ahí"* deja de tener red: **parar depende de que el agente se acuerde de una regla en prosa**, y este mismo catálogo dice que una regla que depende de que alguien se acuerde no es una regla.
>
> **Consecuencia operativa, y hay que comprobarla asunto por asunto:** en permisos amplios, **la puerta humana solo existe donde hay un `deny`**. Y el punto ciego típico no son los comandos —esos suelen estar cubiertos— sino **las herramientas de servidores externos conectados**, que pueden actuar sobre **el mundo físico o sobre servicios reales** y casi nunca aparecen en las listas de denegación, porque se añadieron pensando en el sistema de ficheros.
>
> **Caso medido (2026-08-14):** un asunto con un servidor de domótica conectado tenía **cero** denegaciones de sus herramientas, mientras su fichero de reglas declaraba puerta humana —en prosa— para reiniciar el servidor, desemparejar dispositivos o desarmar la alarma. **La exfiltración de datos y el histórico estaban protegidos; lo que actúa sobre el mundo, no.**
>
> **Lo que hay que hacer con eso NO es una receta: es una elección, y la hace el titular.** Enseñar el hueco es obligación del coordinador; decidir dónde va la frontera, no. En ese caso concreto, **el titular decidió abrirla del todo** en lo técnico —configuración, servicios, dispositivos— y **reservar las denegaciones para comprometer dinero**, con dos argumentos que conviene no perder:
>
> - **La restricción va en la capa del RECURSO, no en la del agente.** Un usuario sin permiso de borrado o un extremo cerrado es una barrera **real, independiente del agente y que ningún prompt sortea**. Una denegación en la configuración del cliente es **frágil** —depende de qué fichero esté cargado— **y estorba el uso legítimo de la misma herramienta**, porque no distingue el caso bueno del malo.
> - **Pedir permiso para algo técnico tiene un coste que no se contabiliza:** convierte a la persona en cuello de botella de su propio sistema, que suele ser justo lo que se estaba intentando quitar.
>
> **La regla que queda, y es la única universal:** en permisos amplios **la puerta humana solo existe donde hay un `deny`**, así que **hay que saber qué hay dentro y qué fuera, y que sea a propósito**. Si la frontera se pone solo en el dinero, perfecto — pero **escrito**, no por descuido. Lo que no vale es creer que protege una regla en prosa que nada ejecuta.
...
## Autonomía desatendida: SECUENCIAL, y con el cortacircuito que de verdad salta

*(Confirmado por dos investigaciones independientes, 2026-08-14.)*

**El paralelismo es destructivo bajo una sola suscripción de consumidor.** Todas las sesiones **comparten la misma cuota** —ventana rodante corta más tope semanal—, así que **N sesiones consumen N veces**, y cada iteración de un agente retransmite su historial entero. Un puñado de procesos simultáneos agota la ventana y **para todo a la vez**, no solo al último. **Por eso la orquestación desatendida se hace con cola y bloqueo: un proceso pesado a la vez.** No es prudencia, es la única forma de que el conjunto no se detenga.

**Y el cortacircuito que salta primero no es el que se diseña.** Se ponen topes de turnos, de tiempo y de gasto, pero en la práctica **el primero que interrumpe es el límite de la plataforma**, que actúa como freno bruto. Los que hay que poner igualmente, porque el freno bruto llega tarde: **tope de turnos**, **tope de tiempo**, y sobre todo **detector de no-progreso**.

> **El fallo silencioso que hay que cazar: agotar la cuota devuelve código de salida 0 con salida vacía.** Un bucle desatendido lo interpreta como éxito y sigue. Es la misma familia que la trampa 2 de la sección anterior —**un `success` es veredicto del runner, no del trabajo**— y se ataja igual: **contar respuestas vacías y comprobar el efecto en el disco**, nunca el código de salida.

**Lo que NO se usa para disparar:** los hooks no pueden lanzar sesiones (ver arriba). Hace falta algo **externo al agente** — planificador del sistema o proceso residente que vigile una carpeta. **Y lo que se lance debe seguir consumiendo la suscripción**: cualquier vía que factura aparte queda fuera por la restricción de siempre.

## Más autonomía no es gratis: lo que se pierde al quitar a la persona del medio

**Hacer de intermediario también es revisar.** Cuando una persona transporta un resultado de una sesión a otra, hace de facto una validación semántica: ve el solape, huele la premisa rara, corta la cadena. **Quitarla del transporte no elimina esa función: la deja vacante.**

Está documentado como **sesgo de automatización** y **síndrome de fuera-del-bucle**: quien deja de ser revisor activo pierde capacidad de detectar el error, y los más confiados en el sistema son los que menos fallos ven. La supervisión útil es **evaluación activa**, no aprobación pasiva.
...
- **Cowork / Claude in Chrome** — tareas de navegador (consultar una web pública, capturas). **Cowork es GA (2026) y está en Team**, pero corre en la **nube de Anthropic** (remote sessions + connectors) y sin audit logs → **frontera de soberanía: datos personales, documentos del asunto y originales NUNCA en Cowork**; solo búsqueda pública o síntesis no sensible. Claude in Chrome (GA en Code/Cowork) opera con **tu sesión real** del navegador → solo sitios de confianza, y **nunca** con una sede electrónica o la banca abiertas.
  - **[INVESTIGADO 2026-08-15, y hay dos cosas que agravan lo anterior]** Primera: **la ejecución en la nube pasó a ser el valor por defecto el 7-jul-2026 y el cambio fue silencioso**, con un interruptor de "ejecutar en la nube" que hay que ir a mirar — o sea que **"corre en local" dejó de poderse suponer y hay que comprobarlo antes de cada uso**. Y lo que se procesa allí incluye **el contenido de los ficheros locales que abra por el puente de la aplicación de escritorio**, no solo lo que se teclee.
  - **[LO QUE MÁS IMPORTA AL MÉTODO, y NO está verificado en fuente oficial] Las reglas de denegación de Claude Code NO gobiernan Cowork.** Tiene su propio modelo de permisos (modos de aprobación + permisos por conector + instrucciones de carpeta), y un `CLAUDE.md` en la carpeta vinculada le llega como **contexto persuasivo, no como barrera**. La evidencia es un caso reproducible publicado en el registro de incidencias del propio fabricante (24-abr-2026), **no documentación**: por eso va etiquetado y **no se propaga como hecho**. Si alguna vez se pilota, esto se comprueba primero, porque **es la barrera sobre la que se apoya todo el kit**.
  - **La regla general que sale de aquí, y esa sí vale para cualquier superficie futura: antes de usar una superficie nueva contra el vault, se comprueban dos cosas — DÓNDE ejecuta y QUÉ barrera la gobierna.** Que una superficie lleve la misma marca que la herramienta de siempre **no significa que herede sus permisos**. Dar eso por hecho es exactamente la familia de error de la lista de permitidos que concede en vez de restringir.
...
> **v1.12 (2026-08-17): las sesiones lanzadas se NOMBRAN, y nombrar no es abrirlas.** Verificado en fuente: una sesión `-p` **enlaza su buzón igual que una interactiva**, así que aparece en el listado y es direccionable por el nombre que le pone `--name`; sin él, el sistema deriva uno de la carpeta y con varias tandas del mismo asunto son indistinguibles. **Se le pone nombre a toda tanda**, por tres motivos: reconocerla en vuelo, poder dirigirse a ella si alguna vez procede, y reanudarla por nombre. **Lo que NO cambia es el reparto del canal:** las ejecutoras siguen en `refuse` porque se rigen por un contrato cerrado — **nombrar y escuchar son cosas distintas**. Y se declara el coste medido de esa decisión: existe un mecanismo de **aviso cuando una sesión queda inactiva**, y con `refuse` **no llega**, porque los controles de entrada se le aplican igual que a un mensaje; el precio de que la tanda sea verificable es ir a buscar su resultado, que es lo que el coordinador ya hace comprobando el disco. **Añadido también el estilo de salida conciso**, fijado en los perfiles como todo lo demás: se elige por fichero y no en un desplegable, y **no alcanza a los subagentes**, que corren con su propio prompt de sistema.
> **v1.11 (2026-08-15): una superficie de la misma marca NO hereda tus barreras, y "corre en local" dejó de poderse suponer.** Del informe sobre las superficies oficiales: la ejecución en la nube pasó a ser el valor por defecto el **7-jul-2026** con un cambio **silencioso**, y lo que se procesa fuera incluye **el contenido de los ficheros locales que la superficie abra**. Y el punto que toca la línea de flotación del método: **las reglas de denegación de Claude Code no gobiernan Cowork**, que tiene su propio modelo de permisos y trata un `CLAUDE.md` de la carpeta como contexto, no como barrera — **etiquetado como NO verificado en fuente oficial**, porque descansa en un caso reproducible del registro de incidencias del fabricante y no en documentación, y el kit ya sabe lo que cuesta propagar sin verificar. De ahí la regla que vale para cualquier superficie que venga después: **antes de usarla contra el vault se comprueba dónde ejecuta y qué barrera la gobierna**, porque compartir marca no es heredar permisos. **Decisión tomada con el informe delante: no se adopta ninguna superficie nueva y no se crea ninguna carpeta para ellas** — el mecanismo no la exige, y sin trabajo real que la necesite sería estructura por adelantado.
> **v1.9 (2026-08-14): la capa de aprobación que muchos dan por hecha puede no existir.** Si las sesiones se arrancan en **permisos amplios** —decisión razonable y frecuente—, **el sistema no pregunta nunca**, y todo lo escrito como *"prepara y para ahí"* se queda sin red: parar pasa a depender de que el agente se acuerde. **En permisos amplios, la puerta humana solo existe donde hay un `deny`.** Y el punto ciego no son los comandos, que suelen estar cubiertos, sino **las herramientas de servidores externos**, que actúan sobre el mundo real y casi nunca entran en las listas de denegación porque estas se escribieron pensando en ficheros. Caso medido: un asunto con servidor de domótica tenía **cero** denegaciones de sus herramientas mientras declaraba puerta humana en prosa para reiniciar el servidor o desarmar la alarma. **Corrige además un cierre erróneo del mismo día:** se dio por zanjado el modo de permisos alegando que el perfil fija `defaultMode`, cuando **un arranque en permisos amplios gana al fichero**. **Y el titular decidió la frontera el mismo día, en contra de lo que yo proponía y con mejor argumento:** libertad **técnica** total —configuración, servicios, dispositivos— y denegaciones **solo para comprometer dinero**, porque **la restricción va en la capa del recurso** (un usuario acotado, un extremo cerrado) y no en la configuración del agente, que es frágil y estorba el uso legítimo. Por eso esta doctrina **enseña el hueco pero no receta la respuesta**: mostrarlo es del coordinador, decidirlo es del titular.
> **v1.8 (2026-08-14): la autonomía desatendida es SECUENCIAL, y quitar a la persona del medio deja una vacante que hay que cubrir.** De dos investigaciones **independientes** sobre el mismo encargo, que coinciden: bajo una sola suscripción **todas las sesiones comparten cuota**, así que el paralelismo no es arriesgado sino **destructivo** — se orquesta con cola y bloqueo. El cortacircuito que salta primero es el de la plataforma, no el que uno diseña, y **agotar la cuota devuelve código 0 con salida vacía**, que es la misma familia que el `success` falso del runner. Y la parte incómoda, confirmada: **hacer de intermediario también es revisar** (sesgo de automatización, síndrome de fuera-del-bucle); al automatizar un relevo el fallo pasa de **duplicado visible a duplicado invisible**, así que la puerta de revisión se **mueve**, no se quita. Con el respaldo de MAST (arXiv:2503.13657, NeurIPS 2025): **el 41,8 % de los fallos multiagente son de especificación y el 36,9 % de coordinación** — casi cuatro de cada cinco—, y *"mejoras en el modelo base serán insuficientes"*. **Ese es el argumento para no debilitar nunca el contrato escrito.**

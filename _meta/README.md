...
- **[verificar-kit.mjs](verificar-kit.mjs)** — el verificador del kit (`node _meta/verificar-kit.mjs`), **ocho reglas**: frontmatter y pie de las doctrinas, índices cuadrados con sus ficheros en los dos sentidos, wikilinks que resuelven, portabilidad, emojis en el kit, vocabulario del core y **redacciones retiradas** — las que un cambio de política dejó obsoletas y que, sin esto, sobreviven donde nadie comprueba. Se ejecuta **solo, sin tubería**, o su código de salida queda enmascarado.
- **[guia-configuracion-agentica-exportable.md](guia-configuracion-agentica-exportable.md)** — la capa de **configuración**, para llevarse a otro vault: los perfiles de permisos con el porqué de cada clave y sus caveats de portabilidad, la **mensajería entre sesiones** completa (reparto por rol, qué no puede hacer un mensaje, y el límite de que solo alcanza sesiones vivas), las **superficies agénticas** con veredicto, las **cuatro trampas medidas** de las sesiones hijas, dónde está de verdad el gasto de contexto, y lo descartado con su umbral. **Complementa al informe del método: aquel explica las reglas, este cómo se configuran.**

...
- **ANTES de eso, la pregunta que decide si esto es un asunto o son dos.** Pongamos un caso: la obra del baño y la reclamación al seguro que la provocó. ¿Van en el mismo contenedor o en dos? Lo decide esta pregunta: **¿las dos tienen que tocar la misma cosa del mundo real?** El mismo presupuesto, el mismo aparato, el mismo contrato.
  - **Si la respuesta es sí, es un solo asunto y un solo contenedor.** Separarlos obliga a **copiar cada dato de un lado al otro** cada vez que algo cambia, y el día que se olvide habrá dos versiones de la misma cifra sin forma de saber cuál vale.
  - **Una pista para cuando dudes, y es la más útil: si hay que explicar por escrito dónde acaba uno y empieza el otro, probablemente son el mismo.** Cuando de verdad son dos cosas distintas no hace falta explicar nada, se ve solo.
  - **Coste medido de equivocarse:** dos asuntos separados por un límite que hubo que declarar por escrito llegaron a mantener **investigaciones simultáneas sobre el mismo equipo físico durante una semana**, con premisas contradictorias y sin que ninguno pudiera verlo. *(Confirmado además por dos investigaciones independientes, 2026-08-14: partir por donde no toca convierte cada contenedor en una isla donde el agente solo ve un fragmento del sistema.)*
  - **Y si comparten esa cosa del mundo real pero aun así quieres dos contenedores:** entonces el límite entre ambos se declara **con árbitro** — se escribe **quién decide cuando choquen, antes de que choquen**.
...
- **Las doctrinas NO se instalan: el catálogo se LEE.** El `settings.json` del paso 4 ya le da `../../general` en solo lectura, y un `[[wikilink]]` sin copia local **resuelve al catálogo, que es lo correcto**. `memoria/` queda para las doctrinas **propias** del asunto. Se copia solo para **fijar** una versión a propósito (diciendo por qué) o si el contenedor va a **salir** del vault; quien copia asume traer los cambios posteriores ([`general/comun/README.md`](../general/comun/README.md), [[estructura_contenedor_asunto]]).
- **El mínimo de reglas que el charter debe citar** para que el coordinador las lea desde el primer día: perfil de modelos ([[modelo_por_tarea]]), higiene de contexto ([[higiene_contexto_y_tokens]]), estructura del contenedor ([[estructura_contenedor_asunto]]) y verificación por el agente ([[verificacion_e2e_por_agente]]). Si el asunto lleva tablero, con el plugin **Bases** de Obsidian (nota-por-tarjeta), no con una aplicación de base de datos propietaria.

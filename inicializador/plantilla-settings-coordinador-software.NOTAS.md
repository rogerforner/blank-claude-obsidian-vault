...
- **El catálogo `general/` sigue en solo lectura** (`Write`/`Edit(//**/general/**)`), con la misma salvedad: `Write(ruta)` es inerte (solo se evalúa `Edit(ruta)`); el par se mantiene por convención, lo que protege es el segundo.
